    ...
    for csv_file in os.listdir("marker_count/" + subdir_name):
        ...
        for ix in range(len(distances_np)):
            ...
            for ix_use in range(1, len(sort_distances), int(np.floor(len(sort_distances) // 19))):
                ...
                usable_indexes_all["compare_to"][-1]["similar"].append({"order": ix_use, "compare_vehicle": vehicle_use, "compare_ride": ride_use, "distance": sort_distances[ix_use][0]})
                
        print(usable_indexes_all)  

        if not os.path.isdir("count_me/" + subdir_name):
            os.makedirs("count_me/" + subdir_name)

        with open("count_me/" + subdir_name + "/" + csv_file.replace("csv", "json"), "w") as outfile: 
