...
df_use_choose = pd.read_csv("new_df_use_choose.csv")
df_use_euclid = pd.read_csv("new_df_use_euclid.csv")
dict_how_many_classes_merged = pd.read_csv("new_dict_how_many_classes_merged.csv")
...
for num_clus in list_clus_short:
    ...
    for algo in algo_list_short:
        ...
        v31 = np.round(np.average(avg_avg_avg_user) * 100, 2)
        avg_2user = dict()
        avg_2avg_2user = dict()
...
for r1 in dict_actual:
    for r2 in dict_actual[r1]:
        ...
        all_ord.append(ord)
print(np.round(np.average(all_euclid), 4), np.round(np.average(all_ord), 2))