...
plt.plot(long_new2, lat_new2, linewidth = 2, label = "Trajectory")
ofs = {1: 0.2, 2: -0.2}
...
for ix in [1, 2]:
    for ws in [10, 20]:
        ...
        y1 = [lat_new2[ix]]
        if ws == 10:
            plt.scatter(x1, y1, c = color_window[ix], marker = start_marker_window[ix], zorder = 3, label = "Start of window " + str(ix))
        else:
            plt.scatter(x1, y1, c = color_window[ix], marker = start_marker_window[ix], zorder = 3)

        x2 = [long_new2[ix], long_new2[ix]]
        y2 = [lat_new2[ix], lat_new2[ix] + ofs[ix] * ws / 10]
        plt.plot(x2, y2, c = color_window[ix], linestyle = linestyle_window[ix][ws])
        
        x3 = [long_new2[ix + ws], long_new2[ix + ws]]
        y3 = [lat_new2[ix + ws], lat_new2[ix + ws] + ofs[ix] * ws / 10]
        plt.plot(x3, y3, c = color_window[ix], linestyle = linestyle_window[ix][ws], label = "Points in window " + str(ix) + " for window size " + str(ws))
        
        x4 = [long_new2[ix + ws]]
        y4 = [lat_new2[ix + ws]]
        plt.scatter(x4, y4, c = color_window[ix], marker = end_marker_window[ix][ws], zorder = 3, label = "End of window " + str(ix) + " for window size " + str(ws))
        
        for nix in range(ix, ix + ws):
            x5 = [long_new2[nix], long_new2[nix + 1]]
            y5 = [lat_new2[nix] + ofs[ix] * ws / 10, lat_new2[nix + 1] + ofs[ix] * ws / 10]
            plt.plot(x5, y5, c = color_window[ix], linestyle = linestyle_window[ix][ws])

plt.legend(ncol = 2, loc = "upper left")
plt.savefig("splitting_new.png", bbox_inches = "tight")
plt.close()