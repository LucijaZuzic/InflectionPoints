...
for subdir_name in os.listdir("marker_count"):
    ...
    for csv_file in os.listdir("marker_count/" + subdir_name):
        ...
        for num_clus in list_clus:
            ...
            if not os.path.isdir("clustered/KMeans/" + str(num_clus) + "/" + subdir_name):
                
                os.makedirs("clustered/KMeans/" + str(num_clus) + "/" + subdir_name)  
            
            new_name = "clustered/KMeans/" + str(num_clus) + "/" + subdir_name + "/clustered_KMeans_" + str(num_clus) + "_" + csv_file
            new_name_png = new_name.replace("csv", "png")
                    
            kmeans = KMeans(n_clusters=num_clus, random_state=0, n_init="auto").fit(X)
