...
def draw_dx(o, x0, y0, dx1, dx2, dy1, dy2):
    ...
    plt.plot([0, 0], [miny, maxy], c = "k", linestyle = "dashed")
    plt.plot([minx, maxx], [0, 0], c = "k", linestyle = "dashed")
    if y2 > y0 or y1 > y0:
    ...
    plt.plot([x0, x1, x2], [y0, y1, y2], c = "k")
    plt.scatter([x0], [y0], c = "r", zorder = 2)
    plt.scatter([x1], [y1], c = "g", zorder = 2)
    plt.scatter([x2], [y2], c = "b", zorder = 2)
    ...
    for xd2 in range(2):
        for yd1 in range(2):
            for yd2 in range(2):
                ...
                ord += 1
plt.savefig("class_illustrate.png", bbox_inches = "tight")
plt.close()
...
make_arrow(0, maxy, 0.1, angle = np.pi / 2)
make_arrow(maxx, 0, 0.1, angle = np.pi * 2)
plt.scatter(long_new2, lat_new2, c = "b", zorder = 2)
...
for num in infls_long:
    plt.scatter(long_new2[num], lat_new2[num], c = "r", zorder = 2)
for num in infls_lat:
    plt.scatter(long_new2[num], lat_new2[num], c = "r", zorder = 2)
plt.savefig("rotate_illustrate.png", bbox_inches = "tight")
plt.close()